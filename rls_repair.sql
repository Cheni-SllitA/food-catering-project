-- =====================================================================
-- RLS REPAIR — fully idempotent. Run this whole file in the Supabase
-- SQL editor. Unlike the original rls_policies.sql, every policy here is
-- dropped-if-exists before being (re)created, so it completes cleanly
-- regardless of which policies already exist. Safe to re-run any time.
--
-- Matches the live schema: role enum values are
--   'customer', 'staff', 'catering_manager', 'administrator'
-- and the role helper is public.my_role().
-- =====================================================================
...
create policy "profiles_select_own" on public.profiles for select
  using (auth.uid() = id);
drop policy if exists "profiles_select_staff_admin_cm" on public.profiles;
...
drop policy if exists "food_categories_select_all" on public.food_categories;
create policy "food_categories_select_all" on public.food_categories for select using (true);
drop policy if exists "food_categories_write_admin" on public.food_categories;
...
drop policy if exists "packages_select_active" on public.catering_packages;
create policy "packages_select_active" on public.catering_packages for select using (is_active = true);
drop policy if exists "packages_select_staff" on public.catering_packages;
...
drop policy if exists "packages_write_admin_cm" on public.catering_packages;
...
drop policy if exists "packages_update_admin_cm" on public.catering_packages;
...
drop policy if exists "packages_delete_admin_cm" on public.catering_packages;
...
drop policy if exists "package_items_write_admin_cm" on public.package_items;
create policy "package_items_write_admin_cm" on public.package_items for insert
  with check (public.my_role() in ('administrator','catering_manager'));
drop policy if exists "package_items_update_admin_cm" on public.package_items;
create policy "package_items_update_admin_cm" on public.package_items for update
  using (public.my_role() in ('administrator','catering_manager')) with check (public.my_role() in ('administrator','catering_manager'));
drop policy if exists "package_items_delete_admin_cm" on public.package_items;
...
drop policy if exists "reservations_select_own" on public.catering_reservations;
create policy "reservations_select_own" on public.catering_reservations for select
  using (customer_id = auth.uid());
drop policy if exists "reservations_select_staff" on public.catering_reservations;
...
drop policy if exists "reservations_insert_own" on public.catering_reservations;
...
drop policy if exists "reservations_update_own" on public.catering_reservations;
create policy "reservations_update_own" on public.catering_reservations for update
  using (customer_id = auth.uid()) with check (customer_id = auth.uid());
drop policy if exists "reservations_update_admin_cm" on public.catering_reservations;
create policy "reservations_update_admin_cm" on public.catering_reservations for update
  using (public.my_role() in ('administrator','catering_manager')) with check (public.my_role() in ('administrator','catering_manager'));
drop policy if exists "reservations_delete_admin" on public.catering_reservations;
...
drop policy if exists "event_bookings_select_staff" on public.event_bookings;
...
drop policy if exists "event_bookings_update_admin_cm" on public.event_bookings;
create policy "event_bookings_update_admin_cm" on public.event_bookings for update
  using (public.my_role() in ('administrator','catering_manager')) with check (public.my_role() in ('administrator','catering_manager'));
drop policy if exists "event_bookings_delete_admin_cm" on public.event_bookings;
...
drop policy if exists "order_items_select_own" on public.product_order_items;
...
drop policy if exists "order_items_select_admin" on public.product_order_items;
...
drop policy if exists "inventory_insert_admin" on public.inventory_transactions;
...
-- ---------------------------------------------------------------------
-- Verify: this should list policies for ALL 15 tables.
-- ---------------------------------------------------------------------
